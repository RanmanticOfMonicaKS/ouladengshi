...
function initPage() {
	...
	$(".cldiv").hide();
	if(userinfo.role_id == '9597cf75078e4599a7872554a53ce2ee'){
	...
	$("input[name='base_user']").val(userinfo.username);
	...
	//选择客户
	if(userinfo.role_id != '9597cf75078e4599a7872554a53ce2ee'){
		$("#base_user").click(function() {
			api.openWin({
				name : 'chose_yh',
				url : '../html/chose_yh.html',
				pageParam : {
					"khmc" : $("#base_user").val(),
					"name" : "kh_edit"
				}
	...
	$("#save_").click(function() {
		...
		GbApi.remote({
			showProgress : false,
			data : {
				"ACTION_NAME" : "sqlKeyBiz.list#customerNameQuery",
				"ACTION_INFO" : paramJson
			},
			done : function(ret) {
				var results = ret.ACTION_INFO.RESULT_LIST;
				if (results.length > 0) {
					$.toptip('客户名称重复', 'warning');
				}else{
					_save();
				}
			}
	...
	$(".weui-uploader__input-box").click(function() {
		GbApi.getPiture(function(ret) {
			if (ret) {
				$("#uploaderFiles").append('<li class="weui-uploader__file" data-type="2" data-url="" onclick="showBig(this)" style="background-image:url(' + ret + ')"></li>');
			}
		});
	});
...
function initData(result) {
	$("#base_user").val(result.xsname);
	$("input[name='base_user']").val(result.base_user);
	...
	for (var i = 0; i < imgUrls.length; i++) {
        if (imgUrls[i])
		$("#uploaderFiles").append('<li class="weui-uploader__file" data-url="'+imgUrls[i]+'" data-type="1" onclick="showBig(this)" style="background-image:url(' + imgUrls[i] + ')"></li>');
	}
}
...
//	}
	...
	if (paramJson.ziti_type == 1) {
	   if($(".form_clxx .car-name").length<=0) {
		...
		paramJson.deleteCars = deleteCars.join();
		//处理图片
...
function deleteCar(obj,name) {
	$(obj).closest(".weui-cell").remove();
	deleteCars.push(name);
}

function callbackChoseYh(id, name) {
	$("#base_user").val(name);
	$("input[name='base_user']").val(id);
}
